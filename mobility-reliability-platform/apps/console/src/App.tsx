...
type WorkflowStage = 'new' | 'assigned' | 'submitted' | 'verified'
...
const initialRepairs = [
  { id: 'SR-2026-081', user: '박정호', device: 'MOB-23991', issue: '주행 중 좌측 쏠림', request: '오늘 08:35', partner: '미배정', amount: '₩180,000', stage: 'new' as WorkflowStage, priority: '높음' },
  { id: 'SR-2026-079', user: '이경자', device: 'MOB-23874', issue: '충전 단자 접촉 불량', request: '어제 16:10', partner: '한마음 모빌리티', amount: '₩95,000', stage: 'assigned' as WorkflowStage, priority: '보통' },
  { id: 'SR-2026-074', user: '최민수', device: 'MOB-23703', issue: '등받이 고정 레버 교체', request: '08. 10. 11:22', partner: '케어휠 수리소', amount: '₩72,000', stage: 'submitted' as WorkflowStage, priority: '보통' },
  { id: 'SR-2026-069', user: '김서윤', device: 'MOB-24018', issue: '타이어 마모 점검', request: '08. 08. 14:05', partner: '한마음 모빌리티', amount: '₩58,000', stage: 'verified' as WorkflowStage, priority: '낮음' },
]
...
function App() {
  ...
  const [selectedRepair, setSelectedRepair] = useState(initialRepairs[0].id)
  ...
  const advanceRepair = () => {
    const currentIndex = workflowOrder.indexOf(activeRepair.stage)
    if (currentIndex >= workflowOrder.length - 1) { notify('이미 센터 검증이 완료된 요청입니다.'); return }
    const nextStage = workflowOrder[currentIndex + 1]
    setRepairs((current) => current.map((repair) => repair.id === activeRepair.id ? { ...repair, stage: nextStage, partner: nextStage === 'assigned' ? '한마음 모빌리티' : repair.partner } : repair))
    notify(`${activeRepair.id} · ${workflowLabels[nextStage]} 단계로 이동했습니다.`)
  }
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      ...
      <div className="content-wrap">
        ...
        {page === 'home' ? <Dashboard activeRepair={activeRepair} repairs={repairs} selectedRepair={selectedRepair} setSelectedRepair={setSelectedRepair} onAdvance={advanceRepair} onNavigate={navigate} notify={notify} /> : <GenericPage page={page} search={search} setSearch={setSearch} users={filteredUsers} repairs={repairs} onNavigate={navigate} notify={notify} />}
      </div>
    </main>
...
function Dashboard({ activeRepair, repairs, selectedRepair, setSelectedRepair, onAdvance, onNavigate, notify }: { activeRepair: typeof initialRepairs[number]; repairs: typeof initialRepairs; selectedRepair: string; setSelectedRepair: (id: string) => void; onAdvance: () => void; onNavigate: (page: PageKey) => void; notify: (message: string) => void }) {
  return <>
    ...
    <div className="metric-grid"><Metric label="처리할 수리 요청" value="7" suffix="건" trend="지난주보다 2건 많아요" tone="orange" icon="repair" onClick={() => onNavigate('repairs')} /><Metric label="예방점검 예정" value="12" suffix="건" trend="이번 주 안에 확인" tone="purple" icon="check" onClick={() => onNavigate('inspections')} /><Metric label="이번 달 지원금" value="₩2,480,000" suffix="" trend="예산의 62% 집행" tone="green" icon="money" onClick={() => onNavigate('subsidy')} /><Metric label="데이터 동기화 상태" value="98.7" suffix="%" trend="최근 24시간 기준" tone="blue" icon="device" onClick={() => onNavigate('system')} /></div>
    <div className="dashboard-grid top-grid"><section className="panel attention-panel"><SectionHeading title="지금 확인이 필요한 일" action="전체 보기" onAction={() => onNavigate('repairs')} /><div className="attention-list"><Attention icon="repair" color="orange" title="새 수리 요청 1건" description="박정호님 · 주행 중 좌측 쏠림" time="8분 전" action="배정하기" onClick={() => onNavigate('repairs')} /><Attention icon="check" color="purple" title="예방점검 예정 3건" description="이번 주 안에 방문 일정을 잡아주세요" time="기한 임박" action="일정 보기" onClick={() => onNavigate('inspections')} /><Attention icon="device" color="blue" title="동기화 대기 기기 1대" description="이경자님 · 마지막 연결 1일 전" time="확인 필요" action="상태 보기" onClick={() => onNavigate('devices')} /></div></section><section className="panel week-panel"><SectionHeading title="이번 주 운영 현황" action="보고서 보기" onAction={() => onNavigate('reports')} /><div className="week-chart"><div className="chart-y"><span>12</span><span>8</span><span>4</span><span>0</span></div><div className="chart-area"><div className="grid-line line-1" /><div className="grid-line line-2" /><div className="grid-line line-3" /><div className="bars"><Bar day="월" value={4} /><Bar day="화" value={7} /><Bar day="수" value={5} /><Bar day="목" value={9} active /><Bar day="금" value={3} muted /><Bar day="토" value={2} muted /><Bar day="일" value={1} muted /></div></div></div><div className="chart-legend"><span><i className="legend-dot completed" />처리 완료</span><span><i className="legend-dot pending" />처리 대기</span><strong>+18% <small>지난주 대비</small></strong></div></section></div>
...
function UsersTable({ users: filteredUsers, notify }: { users: typeof users; notify: (message: string) => void }) {
...
function DevicesTable({ notify }: { notify: (message: string) => void }) {
...
function RepairOperations({ repairs, onNavigate, notify }: { repairs: typeof initialRepairs; onNavigate: (page: PageKey) => void; notify: (message: string) => void }) {
