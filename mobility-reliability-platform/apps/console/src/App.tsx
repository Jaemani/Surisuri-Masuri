...
import calibrationAssessment from '../../../packages/contracts/fixtures/reliability-calibration-assessment.v1.valid.json'
import groundedReportArtifact from './data/r12GroundedReport.json'
import {
...
function ReportsPage() {
  ...
  const reasonLabel = (reason: string) => ({ calibration_sample_insufficient: '검증 표본 부족', reliability_train_insufficient: '기준선 학습 근거 부족', score_variation_insufficient: '점수 다양성 부족', calibration_event_insufficient: '관측 사건 부족' } as Record<string, string>)[reason] ?? '평가 근거 부족'
  ...
  return <>
    ...
    <section className="calibration-readiness" aria-label="합성 보정 가능성 점검"><div className="baseline-section-heading"><div><p className="eyebrow">R11 · CALIBRATION READINESS</p><h2>확률을 보여주기 전에 평가 자격부터 확인합니다</h2></div><StatusPill tone="warning">전체 판단 유보</StatusPill></div><p className="calibration-lead">현재 합성 표본과 부품별 단일 KM 점수로는 보정곡선을 만들 수 없습니다. 수치를 억지로 생성하지 않고 고정 점검 일정과 담당자 검토를 유지합니다.</p><div className="readiness-policy"><span>최소 validation 표본 <strong>{calibrationAssessment.assessmentPolicy.minimumSamples}</strong></span><span>최소 사건 <strong>{calibrationAssessment.assessmentPolicy.minimumEvents}</strong></span><span>최소 점수 종류 <strong>{calibrationAssessment.assessmentPolicy.minimumDistinctScores}</strong></span><span>TEST tuning <strong>사용 안 함</strong></span></div><div className="readiness-grid">{calibrationAssessment.components.map((component) => <article className="readiness-card" key={component.component}><div><span>{component.component.toUpperCase()}</span><StatusPill tone="muted">보정 불가</StatusPill></div><strong>{reasonLabel(component.notEstimableReason)}</strong><dl><div><dt>Validation</dt><dd>n={component.validationCount} · 사건 {component.validationEventCount}</dd></div><div><dt>점수 종류</dt><dd>{component.distinctValidationScoreCount}</dd></div><div><dt>Fallback</dt><dd>고정주기 + 사람 검토</dd></div></dl></article>)}</div><div className="calibration-boundary"><Icon name="shield" size={18} /><span>합성 holdout 평가 harness 검증 · 실제 기기 위험도, 현장 calibration, 개별 운영 조치가 아닙니다.</span></div></section>
    ...
    <section className="grounded-report" aria-label="근거 연결형 합성 보고서"><div className="baseline-section-heading"><div><p className="eyebrow">R12 · EVIDENCE-GROUNDED REPORT</p><h2>문장마다 검증된 Fact ID를 연결합니다</h2></div><StatusPill tone="warning">미발행</StatusPill></div><div className="report-run-state" aria-label="보고서 실행 상태"><div><span>생성 방식</span><strong>Fallback</strong><small>결정론적 template</small></div><i /><div><span>기계 검증</span><strong>통과</strong><small>Fact 5/5 연결</small></div><i /><div><span>사람 검토</span><strong>대기</strong><small>승인 기록 없음</small></div><i /><div><span>발행 상태</span><strong>잠김</strong><small>검토 승인 전 미발행</small></div></div><div className="report-receipt"><span>LLM 사용 안 함 · deterministic fallback</span><code>{groundedReportArtifact.report.reportSha256.slice(0, 16)}…</code></div><div className="grounded-claims">{groundedReportArtifact.report.claims.map((claim) => <article key={claim.claimId}><div><StatusPill tone="success">근거 연결됨</StatusPill><code>{claim.evidenceFactIds[0]}</code></div><p>{claim.text}</p></article>)}</div><div className="calibration-boundary"><Icon name="shield" size={18} /><span>위 문장은 합성 aggregate Fact만 설명합니다. 최종 발행 전 담당자 검토가 필요하며 운영 승인·지원금 집행으로 연결되지 않습니다.</span></div></section>
  </>
}
