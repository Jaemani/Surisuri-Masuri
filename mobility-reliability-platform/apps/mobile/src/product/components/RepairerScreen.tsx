...
function RepairerJobWorkspace({ job, panel, busy, error, onBack, onPanel, onRun, onRefresh }: { job: RepairJob; panel: Panel; busy: boolean; error: string | null; onBack: () => void; onPanel: (panel: Panel) => void; onRun: (command: RepairerJobCommand) => Promise<void>; onRefresh: () => Promise<unknown> }) {
  ...
  return <View style={styles.screen}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {action === 'submit' && panel === 'submit' ? <Card style={styles.formCard}><Text accessibilityRole="header" style={styles.formTitle}>수리 비용 제출</Text><Text style={styles.safeCopy}>현재 계약은 청구 금액만 제출합니다. 복지관이 확인하기 전에는 수리 완료로 처리되지 않습니다.</Text><Text style={styles.inputLabel}>청구 금액</Text><TextInput accessibilityLabel="수리 청구 금액" keyboardType="number-pad" value={amount} onChangeText={(value) => { setAmount(value.replace(/[^0-9]/g, '')); setKey(null); }} placeholder="예: 85000" placeholderTextColor={colors.faint} style={styles.input}/><ProductButton label={busy ? '제출하는 중…' : '금액 확인하고 제출'} disabled={busy || !Number.isSafeInteger(submitAmount) || submitAmount <= 0} onPress={() => void onRun({ action: 'submit', repairRequestId: job.id, expectedRevision: job.revision, billedAmountKrw: submitAmount, idempotencyKey: commandKey() })} /></Card> : null}
...
function statusCopy(status: RepairJob['status']): { label: string; title: string; description: string; tone: 'teal' | 'orange' | 'blue' | 'neutral' } {
  ...
  if (status === 'in_progress') return { label: '작업 중', title: '작업을 마치면 비용을 제출하세요', description: '현재 단계에서는 청구 금액만 제출하며 복지관 검증이 이어집니다.', tone: 'teal' };
