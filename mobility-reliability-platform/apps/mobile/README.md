...
- SQLite schema v2와 v1→v2 atomic migration 구현. 기존 v1 session·event는 보존하되 모두 `local_only/not_applicable`로 고정
...
- batch materializer·digest 계산·HTTP transport와 background 위치 수집은 미착수
...
커밋 전의 unversioned SQLite prototype을 실기기에서 실행한 적이 있다면 현재 앱은 이를 자동 변환하지 않고 안전하게 중단합니다. Version 1 database는 version 2로 transaction migration하며 session·event·installation metadata를 보존하고 기존 delivery metadata는 non-deliverable state로 대체합니다. Android/iPhone의 실제 v1 파일 migration과 앱 재시작은 아직 별도 검증이 필요합니다.
