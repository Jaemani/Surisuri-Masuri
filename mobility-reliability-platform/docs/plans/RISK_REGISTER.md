...
| RSK-30 | cleanup이 late artifact mutation보다 먼저 시작하거나 forward/cleanup owner가 같은 receipt를 경쟁함 | high/low | quiet boundary 전 claim, cleanup receipt의 `next_recovery_at`, context 완료 뒤 추가 create, 복수 cleanup winner, regular·soft-deleted 순차 listing 사이 out-of-band generation | ADR-0023 immutable transition policy, `11m > 5m lease + 5m StoreBatch`, cleanup-only claim; ADR-0024 별도 read/create grant와 create-once target; ADR-0025 concrete fresh delete grant와 raw-first complete-empty audit; ADR-0026 attempt execution ledger·target-after-renewal 금지·원자 finalization 설계; ADR-0027 paired signed read-only evidence와 fresh persistence; ADR-0028 historical/live split, progress 보존 closure, pristine takeover와 target/index 불변 | Phase executor·terminal response-loss code evidence, staging versioning·soft-delete·lifecycle·retention, 모든 writer identity와 IAM write exclusion drill 전 actual delete runtime 및 `expired` completion 금지. 두 listing을 atomic snapshot 또는 point-in-time proof로 해석하지 않음 | active |
