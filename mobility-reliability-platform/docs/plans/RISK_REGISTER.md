...
| RSK-06 | 동의 철회·삭제 계보 실패 | critical/medium | withdrawn trip ingest, orphan object | consent revision authorizer와 atomic admission local 검증; ADR-0024 exact-generation dry-run target과 ADR-0025 conditional delete/complete-empty audit local 검증 | 철회를 관측한 pending recovery/sweeper의 새 artifact write=0 evidence 또는 staging actual-delete drill이 없으면 차단; admission commit 뒤 이미 진행 중인 owner는 별도 fence·reconciliation으로 검증 | active |
...
| RSK-10 | partial failure로 object/manifest/receipt 불일치 | high/medium | reserved age, active lease 중복 처리, stale finalizer, orphan·stored-missing, hash·CRC·generation mismatch | 3-way transaction·ADR-0016 immutable artifact; monotonic fence·bounded classifier·forward/cleanup 분리, ADR-0024 target·ADR-0025 raw-first delete/audit와 ADR-0030 terminal 4문서 finalization·read-only correlation local 검증 | stale mutation=0, raw-only recovery, stored-missing hold, cleanup retry·hold 또는 staging lifecycle 중 하나라도 미검증이면 runtime 차단 | active |
...
| RSK-30 | cleanup이 late artifact mutation보다 먼저 시작하거나 forward/cleanup owner가 같은 receipt를 경쟁함 | high/low | quiet boundary 전 claim, cleanup receipt의 `next_recovery_at`, context 완료 뒤 추가 create, 복수 cleanup winner, dispatch/finalizer response loss, regular·soft-deleted 순차 listing 사이 out-of-band generation | ADR-0023 immutable transition policy, `11m > 5m lease + 5m StoreBatch`, cleanup-only claim; ADR-0024 별도 read/create grant와 create-once target; ADR-0025 concrete fresh delete grant와 raw-first complete-empty audit; ADR-0026 attempt execution ledger·target-after-renewal 금지; ADR-0027 paired signed read-only evidence와 fresh persistence; ADR-0028 historical/live split, progress 보존 closure, pristine takeover와 target/index 불변; ADR-0029 dispatch-before-mutation single winner, single-artifact grant, durable `unknown` barrier와 raw-first signed audit sequencing; ADR-0030 immutable-fence-bounded 4문서 finalizer, target 불변과 `committed\|not_committed\|unverifiable` read-only correlation | Retry/hold code evidence, staging versioning·soft-delete·lifecycle·retention, 모든 writer identity와 IAM write exclusion drill 전 actual delete runtime 및 운영 `expired` 전환 금지. Durable dispatch replay는 audit-first 자동복구가 아니라 `dispatch_pending` 안전 정지이며 두 listing을 atomic snapshot 또는 point-in-time proof로 해석하지 않음 | active |
