...
세부 계약은 [ADR-0018](../decisions/ADR-0018-generation-pinned-read-only-classifier.md)을 기준으로 한다. public artifact recovery port는 write port와 분리하며, recovery claim은 artifact permission으로 사용하지 않는다. `forward_recovery`는 current tenant·installation·trip·assignment·precise-location consent를 system policy로 재평가하고 exact grant binding을 확인한 뒤에만 classifier를 호출한다. 이 경계 전에는 Storage call이 0이어야 한다.
