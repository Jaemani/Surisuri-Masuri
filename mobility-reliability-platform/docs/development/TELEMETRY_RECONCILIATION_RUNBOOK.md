...
R8j local terminal composition의 결정·증거는 [ADR-0032](../decisions/ADR-0032-bounded-cleanup-terminal-orchestration.md)와 [EVD-20260723-040](../evidence/2026-07.md#evd-20260723-040--bounded-cleanup-terminal-orchestration)에 분리한다. `TerminalOrchestrator.Run` 전체 Firestore/GCS vertical slice와 runtime은 아직 미연결이다.

expiry cleanup은 일반 finalizer가 시작하지 않는다. `BeginCleanupTransition`이 `reserved + deadline 경과 + active lease 없음 + 3-way linkage 정상`을 transaction에서 확인한다. 만료 lease에 recovery attempt count가 있으면 exact nested attempt의 owner·token·version·started time을 함께 검증하고 `started`를 같은 transaction에서 `failed/lease_expired`로 닫은 뒤 token을 증가시켜 `cleanup_pending`으로 전환한다. Attempt가 누락·변조·completed이면 receipt도 바꾸지 않고 조사 가능한 lease 증거를 보존한다. Application·receipt·attempt read clock 중 가장 이른 시각이 deadline/lease expiry 전이면 `not_ready`다. 현재 구현은 `reservation_expiry + reserved` origin의 transition, cleanup claim, `absent -> planned|hold` dry-run target, generation-pinned local delete/complete-empty audit, cleanup attempt progress persistence, paired signed absence persistence, progress-aware expired takeover, artifact별 durable phase execution, success-only atomic finalization과 phase-preserving retry·hold control까지다. [ADR-0023](../decisions/ADR-0023-fenced-cleanup-lease-claim.md), [EVD-20260722-031](../evidence/2026-07.md#evd-20260722-031--immutable-quiescence와-fenced-cleanup-lease-claim), [ADR-0024](../decisions/ADR-0024-immutable-cleanup-dry-run-target.md), [EVD-20260722-032](../evidence/2026-07.md#evd-20260722-032--sealed-classification과-immutable-cleanup-dry-run-target), [ADR-0025](../decisions/ADR-0025-generation-pinned-cleanup-delete-and-audit.md), [EVD-20260722-033](../evidence/2026-07.md#evd-20260722-033--generation-pinned-cleanup-delete와-complete-empty-audit), [ADR-0026](../decisions/ADR-0026-fenced-cleanup-execution-ledger-and-expiry-finalization.md), [EVD-20260722-034](../evidence/2026-07.md#evd-20260722-034--fenced-cleanup-execution-ledger와-firestore-progress-persistence), [ADR-0027](../decisions/ADR-0027-paired-read-only-cleanup-absence-attestation.md), [EVD-20260722-035](../evidence/2026-07.md#evd-20260722-035--서명된-read-only-cleanup-absence-audit와-firestore-persistence), [ADR-0028](../decisions/ADR-0028-progress-aware-expired-cleanup-takeover.md), [EVD-20260722-036](../evidence/2026-07.md#evd-20260722-036--progress-aware-expired-cleanup-takeover), [ADR-0029](../decisions/ADR-0029-durable-artifact-phase-cleanup-execution.md), [EVD-20260722-037](../evidence/2026-07.md#evd-20260722-037--durable-artifact-phase-cleanup-execution), [ADR-0030](../decisions/ADR-0030-atomic-cleanup-expiry-finalization.md), [EVD-20260722-038](../evidence/2026-07.md#evd-20260722-038--atomic-cleanup-expiry-finalization과-response-loss-correlation), [ADR-0031](../decisions/ADR-0031-phase-preserving-cleanup-retry-hold-disposition.md), [EVD-20260723-039](../evidence/2026-07.md#evd-20260723-039--phase-preserving-cleanup-retryhold-disposition)을 따른다.

R8c delete/audit, R8d ledger foundation, R8e signed absence persistence, R8f progress-aware takeover, R8g artifact phase executor, R8h success-only finalizer, R8i retry·hold control과 R8j terminal orchestrator는 local·synthetic 범위에서 구현됐지만 **현재 runtime 실행 금지**다. Delete 전송과 complete-empty audit를 분리하고 raw audit 전 manifest delete를 금지한다. 아래 상태는 immutable target이 아니라 exact cleanup attempt의 execution ledger다. Generic progress path는 `planned`, dispatch와 delete outcome만 보존하고 absence-confirmed phase를 계속 거부한다. Dedicated R8e path만 Firestore fresh grant와 paired GCS auditor의 signed evidence를 확인해 raw·manifest absence phase를 저장한다. R8g는 durable dispatch winner만 artifact 하나를 mutation하고 outcome persistence 뒤 known outcome만 이 signed path로 전진한다. R8h는 success terminal을 attempt·receipt·두 index에, R8i는 failure terminal을 attempt·receipt에 각각 원자 확정하며 R8j는 sealed intent로 둘 중 하나만 invocation당 최대 한 번 호출한다. Receipt metadata purge 중 nested attempt와 cleanup-target-only linked page는 local 구현됐지만 operator hold release, reserved-origin hold, accepted deletion, rejected side cleanup, integrity finding purge, final receipt/index purge와 scheduler/startup/readiness는 아직 구현·연결되지 않았다.
...
아래 항목은 현재 local phase executor와 terminal gate의 재개 규칙이다. Component 근거는 phase/revision 검증, target create-only 보존, dispatch/delete outcome persistence, signed absence persistence, exact replay write-zero, progress-aware takeover transaction, artifact별 orchestration, atomic success/failure terminal과 sealed terminal composition까지다. R8j는 phase executor 뒤에서 finalizer 또는 disposition 하나를 호출하지만 전체 `cmd/server`·scheduler/startup/readiness runtime에는 연결하지 않았다.

- raw dispatch commit 뒤 grant 응답 또는 process가 유실된 같은 fence replay: mutation이나 audit을 추측해 반복하지 않고 `dispatch_pending`으로 안전 정지한다. Lease expiry 뒤 R8f takeover가 old progress를 보존한 pristine attempt를 만들며 same-fence response-loss correlation은 후속 gate다.
- manifest dispatch commit 뒤 같은 유실: raw absence가 durable하더라도 manifest mutation이나 audit을 추측하지 않고 `dispatch_pending`으로 안전 정지한다.
...
- `verified_empty` target은 targeted=false의 `not_attempted` outcome을 durable하게 기록한 뒤 별도 read-only absence-audit capability로 두 expected path를 fresh 감사한다. Dispatch 응답 유실은 같은 fence에서 audit-first로 추측하지 않고 `dispatch_pending`으로 정지한다. Classification 당시 empty였다는 이유만으로 `expired` 처리하지 않는다.
...
이어 [ADR-0028](../decisions/ADR-0028-progress-aware-expired-cleanup-takeover.md)의 historical/live authority 분리와 progress-aware expired takeover를 구현했다. Old ledger는 takeover 현재시각이 아니라 7개 nonterminal phase의 마지막 persisted timestamp에서 검증하며, prior progress를 보존한 `failed/lease_expired` closure와 receipt fence·revision·attempt count +1, pristine new attempt create를 한 transaction으로 commit한다. Duplicate attempt와 malformed target·plan·clock은 전체 write 0이고 immutable target과 두 uniqueness index는 불변이다. [EVD-20260722-036](../evidence/2026-07.md#evd-20260722-036--progress-aware-expired-cleanup-takeover)의 local/Emulator/clean CI 근거이며 old outcome·absence를 새 provider 권한으로 상속하지 않는다.
...
[ADR-0030](../decisions/ADR-0030-atomic-cleanup-expiry-finalization.md)의 R8h local finalizer는 exact `manifest_absence_confirmed/revision 7`, non-unknown outcome, fresh two-path evidence와 immutable fence deadline을 다시 확인한다. Attempt `completed/outcome=expired`, receipt `expired`와 receipt·두 index의 같은 `purge_eligible_at`을 4문서 transaction으로 commit하고 target write는 0이다. Commit 결과가 불명확해도 mutation을 replay하지 않으며 pre-state-bound query가 fresh read-only grant를 통해 `committed|not_committed|unverifiable`만 반환한다. [EVD-20260722-038](../evidence/2026-07.md#evd-20260722-038--atomic-cleanup-expiry-finalization과-response-loss-correlation)의 local race·Firestore Emulator 근거이고 R8j local orchestrator에는 연결됐지만 runtime에는 아직 연결하지 않았다.

[ADR-0031](../decisions/ADR-0031-phase-preserving-cleanup-retry-hold-disposition.md)의 R8i local failure control은 10개 bounded class를 exhaustive retry·hold policy로 해석하고 allowed phase/revision을 보존한 attempt terminal과 receipt cursor를 2문서 transaction으로 commit한다. Retry는 old fence와 backoff 중 늦은 boundary에서 exact terminal attempt·target·evidence를 다시 확인한 뒤 pristine attempt를 만들고, hold는 review due가 지나도 operator release 없이 자동 claim하지 않는다. Commit 응답 유실은 첫 query를 보존해 read-only로 상관하며 target·두 index는 모든 disposition path에서 write 0이다. [EVD-20260723-039](../evidence/2026-07.md#evd-20260723-039--phase-preserving-cleanup-retryhold-disposition)의 local race·Firestore Emulator 근거이고 R8j local orchestrator에는 연결됐지만 operator hold release·runtime에는 아직 연결하지 않았다.
...
[ADR-0033](../decisions/ADR-0033-fenced-resumable-receipt-linkage-purge.md)의 R8k는 계속 `proposed`지만 R8k-a job admission/fence, R8k-b nested attempt phase와 R8k-c 중 cleanup-target inverse registry·legacy create-only backfill·cleanup-target-only linked page purge는 local 구현됐다. 운영 규칙은 parent receipt·두 uniqueness index를 먼저 삭제하지 않는 것, purge fence 전에는 child delete를 시작하지 않는 것, advisory query 결과만으로 delete하지 않고 current page·lookahead와 exact child를 transaction에서 다시 읽는 것, malformed·foreign·unsupported·nonterminal·post-fence child를 skip하지 않는 것이다. Valid cleanup-target page는 모든 read와 strict pair 검증을 마친 뒤 target+link delete와 `link_cursor`, `target_deleted_count`, revision을 같은 commit으로 전진시킨다. Poison 한 건이나 finding link가 있으면 target/link delete 0, cursor/count 보존 뒤 durable hold로 닫는다. Page commit 응답 유실 뒤 mutation을 재호출하지 않고 exact pre/next job과 pair presence를 read-only로 상관한다. [EVD-20260723-041](../evidence/2026-07.md#evd-20260723-041--fenced-receipt-purge-admission), [EVD-20260723-042](../evidence/2026-07.md#evd-20260723-042--bounded-nested-recovery-attempt-purge), [EVD-20260723-043](../evidence/2026-07.md#evd-20260723-043--cleanup-target-inverse-registry와-legacy-backfill), [EVD-20260723-044](../evidence/2026-07.md#evd-20260723-044--bounded-linked-cleanup-target-purge)의 local/synthetic/Emulator 범위다.

Top-level target/finding은 `receipt_id` query만 믿지 않고 receipt 하위 inverse `purgeLinks`와 같은 transaction으로 생성해야 한다. Cleanup target writer, legacy target backfill과 등록된 cleanup target+link delete는 이 경계를 구현했지만 integrity finding writer/schema/backfill/delete는 아직 없다. Cleanup-target page가 끝나도 fresh empty/unregistered verification, out-of-band writer exclusion, `ready`와 final receipt/index transaction이 남으므로 `purge_eligible_at`, `linked_documents_purging` 진입, legacy page exhaustion 또는 cleanup-target cursor exhaustion을 metadata purge 완료나 global orphan-zero로 해석하지 않는다.
...
- [x] cleanup retry·hold가 실제 phase/revision을 보존하고 attempt+receipt만 원자 commit하며 target·index write 0
- [x] retry exact boundary concurrent winner 1명, pristine new attempt와 old terminal/target 불변
- [x] hold review due 뒤 자동 claim 0, commit-response-loss 뒤 read-only committed correlation
...
- [x] progress-aware expired cleanup takeover — historical/live 분리, 7개 phase-time 검증, progress 보존 closure, pristine new attempt와 duplicate rollback
- [x] dispatch-before-mutation single winner, replay zero grant/write-zero와 single-artifact provider surface
...
- [x] sealed finalization/disposition intent 외 exported result·error에서 terminal 권한 생성 0
...
- [x] cleanup target inverse-link registry와 legacy create-only backfill
- [x] cleanup-target-only target+link bounded atomic delete와 cursor/count/revision 전진
- [x] linked-page poison whole-page hold와 response-loss read-only correlation
- [ ] integrity finding schema/writer/registry/backfill과 linked delete
- [ ] fresh attempt/link empty, unregistered child 0과 `ready` 전환
- [ ] final job+receipt+two-index linkage delete
...
- runtime 미연결 local component도 제품의 검증된 공학 기반을 바꾸면 Product Update에 기록할 수 있다. 단, 배포·사용자 변화 없음, 검증 환경·데이터와 미연결 runtime 범위를 명시하고 production release로 표현하지 않는다.
