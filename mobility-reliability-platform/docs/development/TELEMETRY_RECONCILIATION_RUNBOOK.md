...
[ADR-0033](../decisions/ADR-0033-fenced-resumable-receipt-linkage-purge.md)의 R8k는 계속 `proposed`지만 R8k-a job admission/fence, R8k-b nested attempt phase와 R8k-c 중 cleanup-target inverse registry·legacy create-only backfill은 local 구현됐다. 운영 규칙은 parent receipt·두 uniqueness index를 먼저 삭제하지 않는 것, purge fence 전에는 child delete를 시작하지 않는 것, advisory query 결과만으로 delete하지 않고 current page·lookahead와 exact child를 transaction에서 다시 읽는 것, malformed·foreign·unsupported·nonterminal·post-fence child를 skip하지 않는 것이다. Valid page의 attempt delete와 cursor/count/revision은 같은 commit이고, empty 확인도 fresh exact read에서만 다음 phase로 전환한다. Page·phase commit 응답 유실 뒤 mutation을 추측해 반복하지 않고 봉인된 pre/next state를 read-only로 상관한다. 신규 cleanup target과 inverse link는 같은 transaction에서 생성하며, legacy target page는 tenant-wide name query와 transaction 내 exact 재조회로 missing link만 보충한다. [EVD-20260723-041](../evidence/2026-07.md#evd-20260723-041--fenced-receipt-purge-admission), [EVD-20260723-042](../evidence/2026-07.md#evd-20260723-042--bounded-nested-recovery-attempt-purge), [EVD-20260723-043](../evidence/2026-07.md#evd-20260723-043--cleanup-target-inverse-registry와-legacy-backfill)의 local/synthetic/Emulator 범위다.

Top-level target/finding은 `receipt_id` query만 믿지 않고 receipt 하위 inverse `purgeLinks`와 같은 transaction으로 생성해야 한다. Cleanup target writer와 legacy target backfill은 이 경계를 구현했지만 integrity finding writer/schema/backfill은 아직 없다. Linked child+link delete, fresh empty/unregistered verification, out-of-band writer exclusion과 final receipt/index transaction도 남아 있으므로 `purge_eligible_at`, job admission, `linked_documents_purging` 진입 또는 legacy page exhaustion을 metadata purge 완료나 global orphan-zero로 해석하지 않는다.
