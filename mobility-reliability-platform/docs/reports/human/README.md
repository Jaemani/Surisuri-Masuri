...
- 정기 리포트 ID: `HR-YYYY-MM-A`, `HR-YYYY-MM-B`
- 권장 발행일: 매월 둘째·넷째 주의 사전에 정한 요일
- 파일명: `YYYY-MM-A-short-title.md`, `YYYY-MM-B-short-title.md`
- 계획 섹션은 8개월 전체 계획을 기준으로 미리 작성할 수 있다.
- 실제 섹션은 발행 시점에만 갱신하고, 수행하지 않은 항목은 `미착수`, `진행 중`, `계획 대비 변경` 중 하나로 명시한다.
- 속도가 계획보다 빠르거나 느려도 계획 서술을 실제 성과처럼 바꾸지 않는다. 차이는 `계획 대비 상태`에 설명한다.
...
정기 리포트는 개발 진척이 작아도 생략하지 않는다. 해당 기간의 실험, 설계 결정, 관측 결과, 위험 또는 다음 검증 계획 중 실제로 확인된 항목을 보고한다.
