...
- 다음 current-state forward recovery authorization은 [ADR-0019](./decisions/ADR-0019-current-forward-recovery-authorization.md)에 결정만 고정했다. Claim을 artifact permission으로 간주하지 않고, authoritative receipt와 current tenant·beneficiary membership·installation·trip·assignment·precise-location consent를 같은 read-only Firestore transaction snapshot에서 확인한 뒤 `ingest` domain authorizer만 짧은 opaque grant를 발급한다. 아직 구현·검증·runtime 성과가 아니다.
...
- generation-pinned classifier의 provider-neutral read orchestration과 열 classification은 local/test 범위에서 구현·검증됐다. 그러나 실제 current system recovery/integrity authorizer는 grant를 발급하지 않으며 startup composition, worker, forward reconciler, attempt completion과 bounded sweeper도 없다. `ClaimRecoveryLease`는 control-plane 처리 소유권일 뿐 artifact read/write 권한이 아니다. staging에서 version·soft-delete·IAM·retention 의미를 검증하고 current authorization을 재평가하는 runtime 경계를 구현하기 전에는 worker·scheduler와 연결하거나 readiness를 열지 않는다.
