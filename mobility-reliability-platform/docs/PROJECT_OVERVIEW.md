...
- recovery attempt의 started·completed·failed protocol, stale closure, authorization disposition과 이를 호출하는 bounded candidate worker component는 local/Emulator에서 구현됐다. Cleanup-only claim, expired takeover, reservation-expiry dry-run target과 generation-pinned delete/audit도 local component로 구현됐지만 scheduler·startup·metrics exporter가 없으므로 실행 중인 background worker는 아니다. `BeginHeldCleanup`, `BeginAcceptedDeletion`, `BeginRejectedArtifactCleanup`, ADR-0026 cleanup execution ledger·terminal completion·`expired`·purge 구현은 아직 남아 있다.
