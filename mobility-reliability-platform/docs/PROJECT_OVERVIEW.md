...
- [ADR-0025](./decisions/ADR-0025-generation-pinned-cleanup-delete-and-audit.md)의 R8c local executor도 구현했다. Concrete Firestore store만 current receipt·exact started attempt·persisted target을 다시 읽어 30초 이하 delete grant를 만들고, GCS adapter는 exact generation+metageneration conditional delete와 complete regular/soft-deleted empty audit만 수행한다. Raw unknown/error 뒤 manifest delete 0, soft-deleted/late generation·incomplete inventory fail-closed, inspect/delete 404 분리와 raw-only/manifest-only missing-path 감사를 고정했다. [EVD-20260722-033](./evidence/2026-07.md#evd-20260722-033--generation-pinned-cleanup-delete와-complete-empty-audit)의 local/Emulator/pinned testbench/clean CI 근거이며 success observation은 completion capability가 아니다. Absence audit persistence, attempt completion, cleanup renewal/release, receipt `expired`, purge, staging policy drill과 runtime은 미구현이다.
- [ADR-0026](./decisions/ADR-0026-fenced-cleanup-execution-ledger-and-expiry-finalization.md)의 pure execution ledger와 Firestore progress persistence foundation을 구현했다. Immutable target은 그대로 두고 exact cleanup attempt에 target/plan hash, fence와 receipt revision을 결박한다. Pure domain은 raw·manifest dispatch·delete outcome·audit phase를 단조적으로 검증하고, Firestore는 planned 및 non-audit dispatch/delete outcome만 저장한다. Local race/vet와 Firestore Emulator의 planned initialization·raw dispatch·exact semantic replay write-zero를 확인했고 [EVD-20260722-034](./evidence/2026-07.md#evd-20260722-034--fenced-cleanup-execution-ledger와-firestore-progress-persistence)에 범위를 기록한다. Generic progress API는 별도 read-only absence-audit capability 없이 `confirmed_absent`를 저장하지 못한다. Absence audit, phase-specific executor, progress-bearing takeover, terminal completion·receipt `expired`·response-loss correlation은 아직 구현하지 않았다.
...
- recovery attempt의 started·completed·failed protocol, stale closure, authorization disposition과 이를 호출하는 bounded candidate worker component는 local/Emulator에서 구현됐다. Cleanup-only claim, expired takeover, reservation-expiry dry-run target, generation-pinned delete/audit와 cleanup progress persistence foundation도 local component로 구현됐지만 scheduler·startup·metrics exporter가 없으므로 실행 중인 background worker는 아니다. `BeginHeldCleanup`, `BeginAcceptedDeletion`, `BeginRejectedArtifactCleanup`, read-only absence audit, phase executor, progress-bearing takeover, terminal completion·`expired`·purge 구현은 아직 남아 있다.
