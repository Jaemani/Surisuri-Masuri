...
- 다음 R6의 설계는 [ADR-0020](./decisions/ADR-0020-two-pass-forward-reconciliation.md)에 고정했다. 정상 raw generation을 다시 쓰지 않고 별도 capability로 canonical manifest만 create한 뒤, fresh current authorization과 full classifier를 다시 실행해 exact `valid_complete`일 때만 fenced `stored`로 확정한다. Classification별 release·hold·reject matrix와 attempt completion 원자성도 결정했지만, 이 문서 시점에는 설계만 accepted이며 구현·검증 증거는 아직 없다.
...
- generation-pinned classifier와 current system forward recovery authorizer는 각각 local/Emulator 범위에서 구현·검증됐다. 그러나 accepted-integrity authorizer, startup composition, worker, forward reconciler, attempt completion과 bounded sweeper는 없다. `ClaimRecoveryLease`는 여전히 control-plane 처리 소유권일 뿐 artifact read/write 권한이 아니며 current authorizer 성공이 있어야 forward read grant를 얻는다. Staging에서 version·soft-delete·IAM·retention 의미와 실제 authorization→Storage race를 검증하기 전에는 worker·scheduler와 연결하거나 readiness를 열지 않는다.
