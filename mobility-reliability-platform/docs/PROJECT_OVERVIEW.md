...
- [ADR-0033](./decisions/ADR-0033-fenced-resumable-receipt-linkage-purge.md)은 R8k metadata purge를 tenant+receipt digest job, receipt writer fence, phase별 bounded cursor·atomic page delete, poison hold, read-only response-loss correlation과 마지막 job+receipt+두 index transaction으로 설계한 `proposed` 결정이다. R8k-a/b/c 구현·증거는 아직 없으며 `purge_eligible_at`은 purge 완료가 아니다.
