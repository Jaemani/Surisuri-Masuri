...
- recovery attempt의 started·completed·failed protocol, stale closure와 authorization disposition은 local/Emulator에서 구현됐지만 이를 호출하는 bounded worker execution loop는 없다. `BeginHeldCleanup`, `BeginAcceptedDeletion`, `BeginRejectedArtifactCleanup`, cleanup lease·target·실행·purge도 미구현이다.
- generation-pinned classifier, current system forward recovery authorizer와 denied/unavailable disposition capability는 각각 local/Emulator 범위에서 구현·검증됐다. 그러나 accepted-integrity authorizer, startup composition, worker, forward reconciler orchestration과 bounded sweeper는 없다. `ClaimRecoveryLease`는 여전히 control-plane 처리 소유권일 뿐 artifact read/write 권한이 아니며 current authorizer 성공이 있어야 forward read grant를 얻는다. Staging에서 version·soft-delete·IAM·retention 의미와 실제 authorization→Storage race를 검증하기 전에는 worker·scheduler와 연결하거나 readiness를 열지 않는다.
