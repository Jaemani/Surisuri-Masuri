...
- [ADR-0033](./decisions/ADR-0033-fenced-resumable-receipt-linkage-purge.md)의 R8k-a/b local control plane을 구현했다. R8k-a는 tenant+receipt digest job, strict job/fence codec, job+receipt create-once admission과 read-only response-loss correlation을 제공한다. R8k-b는 synthetic nested `recoveryAttempts`를 bounded page로 발견하고 transaction 안에서 page/lookahead와 exact raw document를 재검증한 뒤 delete와 cursor/count/revision을 함께 commit한다. Strict terminal/historical cleanup union, same-page single winner, poison hold, fresh-empty phase 전환과 response-loss read-only correlation의 local/Firestore Emulator 근거는 [EVD-20260723-042](./evidence/2026-07.md#evd-20260723-042--bounded-nested-recovery-attempt-purge)와 [HR-20260723-33](./reports/human/HR-20260723-33-nested-recovery-attempt-purge.md)에 있다. R8k-c inverse-link registry·legacy backfill·target/finding/final linkage delete, Rules/indexes와 runtime이 남아 ADR은 계속 `proposed`다. `purge_eligible_at`, job admission과 attempt phase 완료는 전체 purge 완료가 아니다.
