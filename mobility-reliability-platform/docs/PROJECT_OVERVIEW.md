...
- recovery attempt의 started·completed·failed protocol, stale closure, authorization disposition과 이를 호출하는 bounded candidate worker component는 local/Emulator에서 구현됐다. Cleanup-only claim, expired takeover와 reservation-expiry dry-run target create도 local component로 구현됐지만 scheduler·startup·metrics exporter가 없으므로 실행 중인 background worker는 아니다. `BeginHeldCleanup`, `BeginAcceptedDeletion`, `BeginRejectedArtifactCleanup`, actual cleanup target 실행·완료·`expired`·purge는 미구현이다.
