...
- [ADR-0033](./decisions/ADR-0033-fenced-resumable-receipt-linkage-purge.md)의 R8k-a local admission/fence를 구현했다. Tenant+receipt digest job과 post-fence revision linkage hash, strict bounded job codec, receipt의 all-or-none purge fence, job+receipt create-once transaction과 read-only response-loss correlation이 포함된다. Firebase demo Emulator에서 concurrent created 1/replayed 1, job·receipt same commit lineage, index write 0과 partial fence mutation 0을 확인했다. [EVD-20260723-041](./evidence/2026-07.md#evd-20260723-041--fenced-receipt-purge-admission)의 local/synthetic 범위이며 R8k-b attempt paging, R8k-c inverse-link/final linkage delete, Rules/indexes와 runtime이 남아 ADR은 계속 `proposed`다. `purge_eligible_at`과 job admission은 purge 완료가 아니다.
