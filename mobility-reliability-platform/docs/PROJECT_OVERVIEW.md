...
- 현재 제품 UI 명칭: `수리수리마수리` — 사용자 제공 브랜드 원본 적용, 상표 출원·법률 검토는 별도
...
전동보장구의 수리 요청, 수리소 작업, 지원금 집행, 기기 이력과 실제 사용량이 서로 연결되지 않아 다음 문제가 생긴다.

- 사용자는 증상을 어디에 접수하고 수리가 어디까지 진행됐는지 확인하기 어렵다.
- 수리사는 기기의 사용량과 과거 수리 맥락 없이 진단하고 작업 결과를 다시 기관에 전달해야 한다.
- 복지관은 요청 접수·수리소 배정·비용 검증·지원금 집행을 서로 다른 양식과 수기·엑셀로 관리한다.
...
스마트폰 GPS와 AI는 제품의 독립된 목적이 아니다. 사용량과 수리 이력을 결합해 점검 시점과 그 근거를 개선하는 보조 레이어다.

## 5. 주요 사용자와 핵심 과업

| 사용자 | 핵심 과업 | 제품 경험 | 다루지 않는 기대 |
| --- | --- | --- | --- |
| 사용자·보호자 | 증상 접수, 수리 진행 확인, 기기·지원금 확인 | 큰 글씨, 증상 중심 최소 입력, 보호자 연결, 이해 가능한 근거 | 완료 수리기록이나 공적 비용을 직접 확정하지 않음 |
| 수리사 | 배정 작업 확인, QR 기기 조회, 진단·부품·비용 제출 | 과거 이력·누적 사용량·구조화 작업 기록 | 모델이 전문 판단을 덮어쓰지 않음 |
| 복지관 담당자 | 요청 접수, 수리소 배정, 수리·지원금 검증, 점검·행정 운영 | 오늘 할 일, 예외 대기열, 감사 가능한 원장, 양식 출력 | 개인 정밀 동선을 기본 제공하지 않음 |
...
1. 사용자·보호자 또는 복지관이 증상 중심으로 수리를 요청한다.
2. 복지관이 사용자·기기·과거 수리와 지원 가능액을 확인하고 수리소를 배정한다. 기관 정책이 허용하면 사용자가 제휴 수리소를 선택할 수 있다.
3. 수리사는 QR로 기기를 확인하고 진단·부품·전압·견적과 작업 결과를 제출한다.
4. 공적 지원 건은 복지관이 수리 증빙과 보조금 예약·집행·본인부담을 검증한다.
5. 사용자는 완료 내용과 비용을 확인하고 문제가 있으면 재개방을 요청한다.
6. 완료 수리·부품·점검 이벤트가 기기 타임라인과 다음 점검주기를 갱신한다.
7. 별도 동의가 있을 때만 스마트폰 사용량을 수집하고 오프라인 보존·멱등 업로드·위치 최소화를 적용한다.
8. 규칙 또는 모델이 수리 이력과 사용량에서 점검 필요 위험을 계산하되 데이터가 부족하면 유보한다.
9. 수리사는 권고를 검토하고 전문 피드백을 별도 이벤트로 기록한다.
10. 보고서 에이전트는 검증된 Fact ID만 설명하며 복지관은 운영 대기열·예산·통계·기관 양식을 관리한다.
...
- [ADR-0028](./decisions/ADR-0028-progress-aware-expired-cleanup-takeover.md)의 R8f progress-aware expired cleanup takeover도 구현했다. Live provider authority와 historical binding reconstruction을 분리하고, old ledger를 7개 nonterminal phase의 마지막 persisted time에서 검증한다. Prior progress를 보존한 `failed/lease_expired` closure, receipt token·revision·attempt count +1과 pristine new attempt create는 같은 transaction으로 commit 또는 rollback되며 immutable target과 두 uniqueness index는 바뀌지 않는다. [EVD-20260722-036](./evidence/2026-07.md#evd-20260722-036--progress-aware-expired-cleanup-takeover)의 local/Emulator/clean CI 근거이고 old outcome·absence는 새 fence의 provider 권한이 아니다.
- [ADR-0029](./decisions/ADR-0029-durable-artifact-phase-cleanup-execution.md)의 R8g durable artifact-phase executor를 구현했다. Raw 또는 manifest 하나의 dispatch를 Firestore에 먼저 기록하고 transaction `applied` winner만 exact provider grant를 받는다. Mutation 30초 상한과 5초 outcome persistence grace를 분리하며 timeout·cancel·unavailable·response-unverifiable은 durable `unknown`으로 안전 정지한다. Known outcome만 paired signed absence audit로 전진하고 raw absence 전 manifest 호출은 0이다. [EVD-20260722-037](./evidence/2026-07.md#evd-20260722-037--durable-artifact-phase-cleanup-execution)의 local/Emulator/testbench/clean CI 근거이며 성공 종점은 `ready_for_finalization`이다. Terminal success의 후속 현재 상태는 아래 ADR-0030에 분리한다.
- [ADR-0030](./decisions/ADR-0030-atomic-cleanup-expiry-finalization.md)의 R8h local success-only finalizer와 read-only response-loss correlation을 구현했다. Exact `manifest_absence_confirmed/revision 7`만 attempt `completed`, receipt `expired`, 두 uniqueness index의 동일 `purge_eligible_at`으로 한 Firestore transaction에서 전이하고 immutable cleanup target은 write하지 않는다. Commit 응답 유실 뒤에는 finalizer mutation을 추측해 반복하지 않고 fresh read로 `committed|not_committed|unverifiable`을 구분한다. [EVD-20260722-038](./evidence/2026-07.md#evd-20260722-038--atomic-cleanup-expiry-finalization과-response-loss-correlation)의 local unit·Firestore Emulator 근거이며 [HR-20260722-29](./reports/human/HR-20260722-29-atomic-cleanup-expiry-finalization.md)에 사람 대상 범위를 기록한다. 이 항목은 success path만 다룬다.
- [ADR-0031](./decisions/ADR-0031-phase-preserving-cleanup-retry-hold-disposition.md)의 R8i local failure control을 구현했다. 10개 error class를 exhaustive retry·hold policy로 고정하고 durable `unknown`의 bounded class를 같은 outcome revision에 남긴다. Allowed phase 2/3/5/6은 success revision으로 올리지 않고 `completed/cleanup_retry|cleanup_hold` terminal에 그대로 보존하며, attempt와 receipt cursor만 2문서 transaction으로 갱신하고 immutable target·두 index는 write 0이다. Retry는 old fence expiry 이상인 cursor에서 exact terminal attempt·target·evidence를 다시 확인해 pristine attempt를 만들고, hold는 review due 뒤에도 자동 claim하지 않는다. Commit 응답 유실은 first query를 보존한 read-only `committed|not_committed|unverifiable` correlation으로 닫는다. [EVD-20260723-039](./evidence/2026-07.md#evd-20260723-039--phase-preserving-cleanup-retryhold-disposition)와 [HR-20260723-30](./reports/human/HR-20260723-30-cleanup-retry-hold-disposition.md)은 local unit/race·Firebase demo Emulator 범위다.
- [ADR-0032](./decisions/ADR-0032-bounded-cleanup-terminal-orchestration.md)의 R8j local composition을 구현했다. Package-private sealed terminal intent만 `ready_for_finalization` 또는 `ready_for_disposition`을 권한으로 인정하고 success finalizer와 failure disposition의 호출 합계를 invocation당 최대 1회로 제한한다. Outcome·audit persistence ambiguity, unknown mutation status, generic/internal error, conflicting `errors.Join`과 sentinel 사칭은 mutation 0이다. Durable phase 3/6 `unknown` 재시작은 stored class와 exact ledger binding을 복원하며, terminal commit response loss는 mutation 재호출 없이 `context.WithoutCancel` 기반 최대 5초 read-only correlation으로만 닫는다. Result는 receipt·command·query·path를 제외한 bounded DTO이며 direct/correlated lineage를 다시 검증한다. [EVD-20260723-040](./evidence/2026-07.md#evd-20260723-040--bounded-cleanup-terminal-orchestration)과 [HR-20260723-31](./reports/human/HR-20260723-31-cleanup-terminal-orchestration.md)은 local unit/race와 terminal-store Firebase demo Emulator 경쟁 범위다. Operator hold release, accepted/held/rejected cleanup, nested purge와 scheduler/startup/readiness·staging/production Firebase/GCS·actual object delete runtime은 미구현·미연결이다.
- [ADR-0033](./decisions/ADR-0033-fenced-resumable-receipt-linkage-purge.md)의 R8k-a/b와 R8k-c 중 cleanup-target 경로를 local control plane으로 구현했다. R8k-a는 tenant+receipt digest job, strict job/fence codec, job+receipt create-once admission과 read-only response-loss correlation을 제공한다. R8k-b는 synthetic nested `recoveryAttempts`를 bounded page로 발견하고 transaction 안에서 page/lookahead와 exact raw document를 재검증한 뒤 delete와 cursor/count/revision을 함께 commit한다. R8k-c의 현재 범위는 신규 cleanup target+inverse link atomic create, tenant-wide legacy target create-only backfill, 그리고 등록된 cleanup target+link의 bounded atomic delete다. Linked page는 transaction 안에서 current page/lookahead와 exact pair를 다시 읽고, poison 한 건이면 page 전체 delete를 0으로 만들며, valid target+link delete와 `link_cursor`·`target_deleted_count`·revision을 같은 commit으로 전진시킨다. 근거는 [EVD-20260723-043](./evidence/2026-07.md#evd-20260723-043--cleanup-target-inverse-registry와-legacy-backfill), [EVD-20260723-044](./evidence/2026-07.md#evd-20260723-044--bounded-linked-cleanup-target-purge), [HR-20260723-35](./reports/human/HR-20260723-35-linked-cleanup-target-purge.md)에 있다. Integrity finding schema/writer/backfill/delete, fresh empty/unregistered verification, `ready`, final receipt·두 uniqueness index delete와 runtime이 남아 ADR은 계속 `proposed`다. Local client Rules deny는 검증했지만 production Rules·index readiness와 Admin/IAM writer exclusion은 미검증이다. 이 부분 구현을 전체 purge 완료나 global orphan-zero로 해석하지 않는다.
