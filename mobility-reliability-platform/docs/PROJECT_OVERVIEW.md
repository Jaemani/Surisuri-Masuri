...
- generation-pinned classifier의 raw/manifest content validator와 codec registry는 pure local boundary로 구현됐지만, GCS inventory/read와 결합한 최종 분류 orchestration, forward reconciler와 bounded sweeper runtime은 아직 구현·검증되지 않았다. Storage reader·content validator의 `verified` 범위도 synthetic·testbench·clean runner 계약일 뿐 authorization이나 classifier 완료가 아니다. `ClaimRecoveryLease`는 control-plane 처리 소유권일 뿐 artifact read/write 권한이 아니다. current system authorization을 재평가하는 runtime 경계가 구현되기 전에는 worker·scheduler와 연결하지 않는다.
