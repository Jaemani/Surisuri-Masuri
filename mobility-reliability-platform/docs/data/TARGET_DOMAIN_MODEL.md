...
- `cleanup_pending`은 `BeginCleanupTransition` 또는 `BeginHeldCleanup`이 token을 증가시키고 forward lease를 fence-out한 뒤 늦은 Storage write의 quiet period를 기다리는 reserved-origin 상태다. Deadline transition은 `cleanup_transitioned_at`, exact quiescence, mode, origin과 policy version을 불변으로 기록한다. Recovery attempt count가 있는 만료 lease는 exact nested attempt를 같은 transaction에서 `failed/lease_expired`로 닫아야 하며, linkage를 검증할 수 없으면 receipt transition도 금지한다. Quiet boundary 뒤 cleanup-only owner가 lease와 `started` attempt를 transaction으로 claim하고 `next_recovery_at`은 제거한다. R8b target create와 R8c local delete/audit는 receipt·lease·attempt를 변경하지 않고 `started`를 유지하며 target이나 success observation 자체는 `expired` 권한이 아니다. 일반 forward finalizer와 accepted/rejected cleanup은 이 상태로 전환할 수 없다. `expired`는 future fresh fenced completion transaction이 durable outcome을 확인한 뒤에만 사용한다.
...
ADR-0026의 reservation-expiry cleanup attempt는 같은 문서에 다음 bounded execution field를 추가한다. Pure ledger와 Firestore DTO/codec·fresh non-audit progress persistence는 구현됐고 [EVD-20260722-034](../evidence/2026-07.md#evd-20260722-034--fenced-cleanup-execution-ledger와-firestore-progress-persistence)에서 local/Emulator 범위를 확인했다. Terminal disposition·finalizer field는 아직 쓰이지 않는다.
