...
4. request의 `tenantId`를 후보 scope로 사용해 `/tenants/{tenantId}/memberships/{uid}`의 활성 상태·유효기간을 확인하고, 필요한 배포에서만 허용 tenant custom claim을 추가 제한으로 교차 확인한다.
5. 서버가 membership role, 사람·기기 관계, trip, installation, consent와 목적을 검사해 scope를 최종 authorize한다. client body의 tenant/role은 단독 권한 근거로 사용하지 않는다.
