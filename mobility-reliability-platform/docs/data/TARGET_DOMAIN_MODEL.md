...
/tenants/{tenantId}
  ...
  /ingestPurgeJobs/{purgeKey}                   # tenant+receipt digest 기반 bounded linkage purge coordinator
...
- `privatePeople`, `appInstallations`, `ingestReceipts`와 하위 `purgeLinks`, `ingestCleanupTargets`, `ingestIntegrityFindings`, `ingestIdempotency`, `ingestClientBatches`, `recoveryWorkerState`, `consentStates`, `domainEvents`, `projectionCheckpoints`, `deadLetters`, `externalIdentifiers`, `dataAccessGrants`, `authzIndex`의 client write는 항상 거절한다. Ingest receipt/link, cleanup target와 integrity finding은 client direct read/list도 거절한다. 이는 integrity finding server writer나 production IAM exclusion이 구현됐다는 뜻이 아니다.
...
cleanup_transitioned_at?, cleanup_quiescence_until?, cleanup_policy_version?,
...
cleanup_disposition_attempt_id?, cleanup_control_disposition?(retry|hold),
...
- `cleanup_pending`은 `BeginCleanupTransition` 또는 `BeginHeldCleanup`이 token을 증가시키고 forward lease를 fence-out한 뒤 늦은 Storage write의 quiet period를 기다리는 reserved-origin 상태다. Deadline transition은 `cleanup_transitioned_at`, exact quiescence, mode, origin과 policy version을 불변으로 기록한다. Recovery attempt count가 있는 만료 lease는 exact nested attempt를 같은 transaction에서 `failed/lease_expired`로 닫아야 하며, linkage를 검증할 수 없으면 receipt transition도 금지한다. Quiet boundary 뒤 cleanup-only owner가 lease와 `started` attempt를 transaction으로 claim하고 `next_recovery_at`은 제거한다. Baseline 또는 active cleanup lease에는 cleanup disposition cursor 다섯 개가 모두 비어 있어야 한다. R8b target create와 R8c local delete/audit는 receipt·lease·attempt를 변경하지 않고 `started`를 유지하며 target이나 success observation 자체는 `expired` 권한이 아니다. R8i retry·hold terminal은 lease를 제거한 뒤 exact terminal attempt를 `cleanup_disposition_attempt_id`로 가리킨다. Retry만 old fence expiry 이상인 `next_cleanup_at`에서 cursor를 검증·제거하고 pristine 새 attempt를 원자 생성할 수 있으며, hold review due는 사람 검토 기한일 뿐 자동 claim 권한이 아니다. 일반 forward finalizer와 accepted/rejected cleanup은 이 상태로 전환할 수 없다. `expired`는 R8h fresh fenced completion transaction이 durable terminal evidence를 확인한 reserved-origin success path에서만 사용한다.
...
```text
schema_version=telemetry-cleanup-target.v1,
cleanup_id, tenant_id, receipt_id, reservation_key, attempt_id,
mode=reservation_expiry, cleanup_origin_status=reserved,
cleanup_policy_version, cleanup_transitioned_at, cleanup_quiescence_until,
receipt_revision, fencing_token,
lease_acquired_at, lease_heartbeat_at, lease_expires_at,
worker_version=telemetry-cleanup.v1,
status(planned|hold), decision(delete_candidate|verified_empty|hold),
classification, reason_code, retention_phase,
validator_version, classified_at,
...
created_at, updated_at, target_hash
```
...
R8d execution state도 target을 in-place update하지 않고 exact cleanup attempt에 기록한다. Delete dispatch와 delete RPC outcome, `confirmed_absent` audit을 분리하며 raw audit 전 manifest dispatch는 0이다. Generic progress persistence는 계속 두 `confirmed_absent` phase를 거부한다. 후속 [ADR-0027](../decisions/ADR-0027-paired-read-only-cleanup-absence-attestation.md)의 dedicated signed path만 Firestore fresh current state에서 만든 30초 이하 read grant, paired GCS auditor의 opaque Ed25519 evidence와 current transaction 재검증을 모두 통과한 raw·manifest audit phase를 저장한다. Exact replay는 persisted `AuditedAt`까지 같아야 write 0이고, 다른 observation time·stale receipt revision/fence/ledger revision은 conflict/write 0이다. 이 write는 exact attempt만 갱신하며 receipt, 두 uniqueness index와 immutable cleanup target은 바꾸지 않는다.

R8g [ADR-0029](../decisions/ADR-0029-durable-artifact-phase-cleanup-execution.md)는 raw 또는 manifest artifact 하나의 dispatch를 provider call보다 먼저 exact attempt에 저장한다. Transaction `applied` winner만 target·plan·receipt revision·fence·artifact·path·lineage와 pre-dispatch revision에 묶인 non-zero grant를 받고 exact replay는 write 0·zero grant다. Single-artifact executor는 30초 이하 mutation window와 뒤따르는 5초 outcome persistence grace 안에서 bounded `not_attempted|observed|not_found_observed|unknown`을 반환한다. Outcome을 별도 durable revision에 저장한 뒤 known outcome만 paired signed absence audit로 전진한다. `unknown`이면 audit와 counterpart artifact가 모두 0이고, durable dispatch replay는 mutation 여부를 추측하지 않은 채 `dispatch_pending`으로 정지한다. 성공 종점도 `manifest_absence_confirmed`와 `ready_for_finalization`이며 receipt·attempt·index를 terminal로 바꾸지 않는다.

`verified_empty/planned`와 dispatch-response-loss 재개는 destructive delete grant가 아니라 current target·fence·expected paths에 결합된 위 read-only absence-audit capability를 사용한다. Classification 당시 empty였다는 사실만으로 완료하지 않고 regular/soft-deleted inventory를 fresh complete-empty로 다시 확인한다. 다만 두 inventory listing은 순차 provider call이므로 atomic snapshot이나 point-in-time proof가 아니다. Post-quiescence application writer fencing과 least-privilege IAM/write exclusion을 staging에서 확인하기 전에는 production completion 근거로 승격하지 않는다. R8h [ADR-0030](../decisions/ADR-0030-atomic-cleanup-expiry-finalization.md)의 fresh finalization transaction만 `manifest_absence_confirmed/revision 7`과 durable two-path audit을 근거로 attempt `completed/outcome=expired`, receipt `expired`와 receipt·두 index의 같은 `purge_eligible_at`을 4문서 원자 commit한다. Immutable target write는 0이며 response-loss outcome read는 `committed|not_committed|unverifiable`만 반환한다.

R8i [ADR-0031](../decisions/ADR-0031-phase-preserving-cleanup-retry-hold-disposition.md)은 execution-stage failure를 success finalizer와 분리한다. Allowed phase 2/3/5/6을 success revision으로 올리지 않고 bounded error class를 exhaustive retry·hold policy로 해석해 attempt와 receipt만 2문서 원자 commit한다. Retry cursor는 `max(old_fence.expires_at, completed_at + backoff)`이며 exact terminal attempt·old target·evidence·fence를 다시 검증한 claim만 cursor를 지우고 pristine attempt를 만든다. Hold due는 자동 claim을 만들지 않는다. Commit 응답이 불명확하면 첫 pre-state query를 보존하고 fresh read-only correlation으로만 `committed|not_committed|unverifiable`을 판별한다.

R8j [ADR-0032](../decisions/ADR-0032-bounded-cleanup-terminal-orchestration.md)는 R8g phase execution과 R8h/R8i terminal store를 package-private sealed intent로 합성한다. Exported `ExecutionResult`, Go error, phase enum이나 error class는 terminal 권한이 아니다. Exact query·fence·target/plan hash·receipt revision·phase/revision과 diagnostic source를 봉인한 intent만 finalization 또는 disposition 하나를 invocation당 최대 한 번 호출한다. Durable `unknown`은 저장된 class와 ledger shape를 authoritative하게 복원하고, outcome/audit persistence ambiguity나 control-store failure에는 intent를 만들지 않는다. Terminal commit response loss는 pre-state-bound query가 있을 때만 cancellation과 분리된 최대 5초 read-only correlation을 수행하며 mutation을 반복하거나 반대 terminal로 fallback하지 않는다.

reserved-origin expiry cleanup은 exact prior forward attempt가 있으면 먼저 같은 transaction에서 terminal로 닫고 receipt를 `cleanup_pending`으로 바꾸며 모든 forward lease를 fence-out한 뒤 `cleanup_quiescence_until`까지 기다린다. Attempt closure와 receipt transition은 함께 commit 또는 rollback되어야 하고 recovery attempt count는 누적값으로 유지한다. 현재 `cleanup_policy_version=telemetry-cleanup-transition.v1`은 transition 11분 뒤 quiescence를 고정하며 이는 최대 lease 5분과 raw+manifest 전체 `StoreBatch` 5분 합보다 strict하게 길다. Quiet boundary 뒤 `telemetry-cleanup.v1` owner만 claim한다. 현재 target create, local delete/audit, dispatch/delete outcome, dedicated signed absence persistence, progress-aware expired takeover, durable artifact phase orchestration, success-only terminal finalizer, phase-preserving retry·hold control, sealed terminal composition과 cleanup-target-only linked purge는 `reservation_expiry + reserved` origin의 local component로 구현됐다. Takeover는 [ADR-0028](../decisions/ADR-0028-progress-aware-expired-cleanup-takeover.md)에 따라 검증된 prior progress를 `failed/lease_expired` attempt에 보존하고 immutable target과 두 uniqueness index를 바꾸지 않은 채 pristine new attempt를 원자 생성한다. Phase orchestration은 [ADR-0029](../decisions/ADR-0029-durable-artifact-phase-cleanup-execution.md)에 따라 dispatch-before-mutation과 `unknown` barrier를 적용한다. Success terminal authority는 [ADR-0030](../decisions/ADR-0030-atomic-cleanup-expiry-finalization.md)의 exact 4문서 transaction, failure terminal authority는 [ADR-0031](../decisions/ADR-0031-phase-preserving-cleanup-retry-hold-disposition.md)의 exact 2문서 transaction에만 있고 [ADR-0032](../decisions/ADR-0032-bounded-cleanup-terminal-orchestration.md)의 orchestrator가 두 경계를 상호 배타적으로 선택한다. Operator hold release, accepted retention cleanup, rejected cleanup, integrity finding linkage/purge, fresh global verification, `ready`, final receipt/index purge와 scheduler/startup/readiness·staging/production runtime은 future 범위이며 새 generation이 보이면 원 target을 바꾸지 않고 finding 또는 별도 linked target으로 분리해야 한다. 자세한 claim 계약은 [ADR-0023](../decisions/ADR-0023-fenced-cleanup-lease-claim.md)을 따른다.

`/ingestPurgeJobs/{purgeKey}`:

```text
schema_version, policy_version,
purge_key, tenant_id, receipt_id, receipt_revision, linkage_hash,
status(planned|attempts_purging|linked_documents_purging|ready|linkage_deleted|hold), revision,
...
created_at, updated_at, held_from_status?, error_class?
```

`purge_key=sha256("ingest-receipt-purge@1\x00" || tenant_id || "\x00" || receipt_id)`이며 receipt ID가 tenant 사이에서 같아도 job을 공유하지 않는다. [ADR-0033](../decisions/ADR-0033-fenced-resumable-receipt-linkage-purge.md)의 R8k-a local 구현은 terminal receipt와 두 index의 exact `purge_eligible_at <= now`를 재검증한 transaction에서 job을 create-once하고 receipt에 `purge_job_id`, `purge_started_at`, `purge_fence_version`을 함께 기록한다. Existing recovery/cleanup lease claim과 cleanup target create는 이 fence 뒤 fail-closed한다. R8k-c의 첫 local 증분에서 cleanup target inverse-link writer와 legacy target backfill을 구현했지만 integrity finding strict DTO/writer는 없으므로 모든 future child writer를 닫았다고 주장하지 않는다.

Nested `recoveryAttempts`는 `__name__` cursor로 직접 제거한다. R8k-b local 구현은 advisory `page_size+1` 결과를 transaction 안에서 current page·lookahead로 재조회하고, 각 exact raw document를 다시 읽어 terminal 또는 허용된 historical `failed/lease_expired` cleanup ledger인지 검증한다. Attempt delete와 job cursor·deleted count·revision 전진은 같은 transaction이고, malformed·foreign·unsupported·nonterminal·post-fence child는 skip하지 않고 delete 0의 bounded `hold`로 남긴다. `planned → attempts_purging`과 fresh exact-empty 뒤 `attempts_purging → linked_documents_purging` 전환도 원자적이며 [EVD-20260723-042](../evidence/2026-07.md#evd-20260723-042--bounded-nested-recovery-attempt-purge)의 synthetic/Emulator 범위에서만 검증됐다.

Top-level cleanup target와 integrity finding create는 receipt 하위 `purgeLinks` inverse registry document를 같은 transaction에서 만들고, purge는 registry를 `__name__` cursor로 열거해 exact linked child+link를 함께 제거해야 한다. `receipt_id` equality query만으로 malformed child까지 발견했다고 주장하지 않는다. Fresh attempt/link empty와 well-formed unregistered child 0을 확인한 뒤에만 `ready`가 되며 마지막 transaction은 job update와 두 uniqueness index·receipt delete를 같은 commit으로 수행한다. Job에는 좌표·Firebase UID·device/trip/person ID·object path를 넣지 않고 최소 count·hash·완료시각만 자체 retention까지 보존한다. 현재 R8k-a/b, cleanup-target registry/legacy backfill과 등록된 cleanup target+link의 bounded page purge가 local 구현됐다. Linked page는 transaction 안에서 current page/lookahead와 strict target/link pair를 다시 읽고, valid target+link delete와 `link_cursor`, `target_deleted_count`, revision을 같은 commit으로 전진시킨다. `target_deleted_count`는 삭제한 cleanup target 수이며 link delete 수를 더하지 않고 `finding_deleted_count`는 현재 증분에서 변하지 않는다. Poison 또는 finding link 한 건이면 page 전체를 hold한다. Backfill의 `ObservedExhausted`와 마지막 non-empty cleanup-target page delete는 global inventory나 `ready` 증거가 아니다. Integrity finding writer/backfill/delete, fresh unregistered verification, `ready`와 final four-document transaction은 남아 있다. 따라서 attempt phase, target backfill 또는 cleanup-target page 완료도 purge 완료 상태가 아니다.
