...
4. `/tenants/{tenantId}/memberships/{uid}`의 활성 상태·유효기간과 필요한 경우 허용 tenant custom claim을 교차 확인한다.
5. 서버가 최종 `tenant_id`, membership role, 사람·기기 관계와 목적을 결정한다. client body의 tenant/role은 권한 근거로 사용하지 않는다.
...
`/appInstallations/{installationId}`는 client UUID, Firebase UID, App Check App ID, 등록·폐기 상태와 최근 확인시각을 연결하는 server-managed 문서다. 하드웨어 fingerprint를 만들지 않으며 재설치·로그아웃·기기 양도 시 새 relation으로 취급한다.
...
- `privatePeople`, `appInstallations`, `ingestReceipts`, `ingestIdempotency`, `ingestClientBatches`, `domainEvents`, `projectionCheckpoints`, `deadLetters`, `externalIdentifiers`, `dataAccessGrants`, `authzIndex`의 client write는 항상 거절한다.
...
`telemetry-batch.v2` request는 하나의 제한된 JSON object이며 deterministic gzip으로 저장한다. request의 identity/reference field는 다음과 같다.

```text
client_batch_id, tenant_id, device_id, trip_id,
client_session_id, installation_id, consent_revision_id, sent_at
```
...
- ingest idempotency key는 `sha256(payload_schema_version + U+001F + tenant_id + U+001F + installation_id + U+001F + client_batch_id)`의 lowercase hex다.
