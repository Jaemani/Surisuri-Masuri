...
R8d execution state도 target을 in-place update하지 않고 exact cleanup attempt에 기록한다. Delete dispatch와 delete RPC outcome, `confirmed_absent` audit을 분리하며 raw audit 전 manifest dispatch는 0이다. 현재 generic progress persistence는 planned·dispatch·delete outcome만 허용하고, caller가 command shape만으로 absence evidence를 만들지 못하도록 두 `confirmed_absent` phase를 거부한다. Fresh terminal transaction만 durable two-path audit을 근거로 receipt `expired`, attempt completed와 세 linkage 문서의 같은 `purge_eligible_at`을 commit한다. Response-loss outcome query는 `committed|not_committed|unverifiable`만 반환해야 한다. 마지막 두 문장의 terminal finalizer와 correlation은 아직 구현 전이다.
