...
Nested `recoveryAttempts`는 `__name__` cursor로 직접 제거한다. Top-level cleanup target와 integrity finding create는 receipt 하위 `purgeLinks` inverse registry document를 같은 transaction에서 만들고, purge는 registry를 `__name__` cursor로 열거해 exact linked child+link를 함께 제거한다. `receipt_id` equality query만으로 malformed child까지 발견했다고 주장하지 않는다. Exact delete와 job cursor/count/revision 전진은 같은 transaction이며 malformed·foreign child는 skip하지 않고 bounded `hold`로 남긴다. Fresh attempt/link empty와 well-formed unregistered child 0을 확인한 뒤에만 `ready`가 되며 마지막 transaction은 job update와 두 uniqueness index·receipt delete를 같은 commit으로 수행한다. Job에는 좌표·Firebase UID·device/trip/person ID·object path를 넣지 않고 최소 count·hash·완료시각만 자체 retention까지 보존한다. 현재 R8k-a job admission/fence만 local 구현됐으며 inverse registry migration·global inventory와 R8k-b/c 구현 증거 전에는 purge 완료 상태가 아니다.
