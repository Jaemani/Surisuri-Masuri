...
- [ADR-0025](./decisions/ADR-0025-generation-pinned-cleanup-delete-and-audit.md)의 concrete Firestore delete grant와 generation+metageneration-pinned GCS executor도 local component로 구현됐다. Raw-first complete-empty audit, missing counterpart path audit, soft-deleted/late generation과 incomplete inventory fail-closed, delete/inspect 404·timeout·permission·quota·412 분리를 [EVD-20260722-033](./evidence/2026-07.md#evd-20260722-033--generation-pinned-cleanup-delete와-complete-empty-audit)에서 local/Emulator/pinned testbench/clean CI로 확인했다. Full success observation은 non-authoritative shape이며 attempt execution/outcome ledger, completion·renewal/release, receipt `expired`, purge와 runtime은 미구현이다.
- [ADR-0026](./decisions/ADR-0026-fenced-cleanup-execution-ledger-and-expiry-finalization.md)은 immutable target을 수정하지 않고 exact cleanup attempt를 dispatch·audit 단계 원장으로 확장하며, fresh completion transaction이 attempt·receipt·두 index를 함께 완료하고 response loss를 read-only correlation으로 판별하도록 결정했다. 이 항목은 accepted design이며 아직 code·test evidence가 없다.
