...
- [ADR-0033](./decisions/ADR-0033-fenced-resumable-receipt-linkage-purge.md)의 R8k-a local gate를 구현했다. Deterministic job/key/hash, strict bounded codec, receipt writer fence, job+receipt create-once transaction과 read-only response-loss correlation이 포함된다. Concurrent admission은 created 1/replayed 1로 수렴하고 두 uniqueness index는 write 0이며 existing recovery/cleanup lease claim과 cleanup target create를 fence 뒤 닫는다. [EVD-20260723-041](./evidence/2026-07.md#evd-20260723-041--fenced-receipt-purge-admission)과 [HR-20260723-32](./reports/human/HR-20260723-32-receipt-purge-admission.md)의 local/synthetic/Emulator 범위다. ADR은 R8k-b/c가 남아 `proposed`이며 child·receipt/index delete, runtime과 production purge는 미구현·미연결이다.
