...
- HTTP-only GCS reader, 분리된 `Versions`/`SoftDeleted` exact-path inventory, bounded `limit+1` 관찰, generation+metageneration read precondition, raw compressed-byte flag와 typed provider error 경계가 `main`에 구현돼 WSL2 Docker local synthetic 전체 Go gate와 clean CI를 통과했다. [EVD-20260721-020](./evidence/2026-07.md)은 `verified`이지만 read-side official Storage testbench·staging·runtime 증거가 아니다. classifier validator·최종 분류와 authorization/runtime composition은 미구현이다.
