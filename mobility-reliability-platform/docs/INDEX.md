...
- [ADR-0033](./decisions/ADR-0033-fenced-resumable-receipt-linkage-purge.md)은 R8k receipt linkage purge를 deterministic job, receipt writer fence, phase별 bounded cursor와 atomic page commit, read-only response-loss correlation, 마지막 job+receipt+두 index transaction으로 설계한 `proposed` 결정이다. 아직 purge job contract·Firestore mutation·child delete·runtime을 구현하지 않았으며 R8k-a/b/c gate별 증거 전에는 삭제 완료로 표현하지 않는다.
