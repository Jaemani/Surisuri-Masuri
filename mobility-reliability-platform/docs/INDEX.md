...
- [ADR-0025](./decisions/ADR-0025-generation-pinned-cleanup-delete-and-audit.md)의 concrete Firestore delete grant와 generation+metageneration-pinned GCS executor도 local component로 구현됐다. Raw-first complete-empty audit, missing counterpart path audit, soft-deleted/late generation과 incomplete inventory fail-closed, delete/inspect 404·timeout·permission·quota·412 분리를 [EVD-20260722-033](./evidence/2026-07.md#evd-20260722-033--generation-pinned-cleanup-delete와-complete-empty-audit)에서 local/Emulator/pinned testbench/clean CI로 확인했다. Full success observation은 non-authoritative shape이며 absence-audit persistence, completion·renewal/release, receipt `expired`, purge와 runtime은 미구현이다.
- [ADR-0026](./decisions/ADR-0026-fenced-cleanup-execution-ledger-and-expiry-finalization.md)의 pure execution ledger와 Firestore persistence foundation도 구현됐다. Canonical plan과 target/fence/receipt revision을 exact cleanup attempt에 고정하고 pure domain에서 전체 dispatch·delete outcome·audit phase/revision을 검증한다. Firestore는 planned와 non-audit dispatch/delete outcome progress만 저장하며, generic API는 read-only audit capability 없이 `confirmed_absent`를 기록하지 못하게 차단한다. Local race/vet와 Firestore Emulator의 planned initialization·raw dispatch·semantic replay write-zero 근거는 [EVD-20260722-034](./evidence/2026-07.md#evd-20260722-034--fenced-cleanup-execution-ledger와-firestore-progress-persistence)에 기록한다. Absence-audit capability, phase executor, progress-aware takeover, terminal `expired` finalizer와 runtime은 아직 미구현이다.
