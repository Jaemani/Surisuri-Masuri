...
```text
schemaVersion + U+001F + tenantId + U+001F + installationId + U+001F + clientBatchId
```
...
- v2 request는 `schemaVersion`, `tenantId`, `deviceId`, `tripId`, `clientSessionId`, `installationId`, `consentRevisionId`, `clientBatchId`, `sentAt`, `samples`만 가진다.
