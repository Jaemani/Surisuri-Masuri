...
[ADR-0026](./ADR-0026-fenced-cleanup-execution-ledger-and-expiry-finalization.md)은 target 생성 뒤의 progress-bearing cleanup attempt에 한해 이 규칙을 좁게 확장한다. Exact immutable target과 단조 execution ledger를 함께 검증한 `started` attempt는 progress를 보존한 `failed/lease_expired`로 닫고 새 claim할 수 있다. Cleanup disposition이나 conflicting/malformed progress가 있으면 기존 규칙대로 write 0이다. Forward attempt와 target 생성 전 empty cleanup attempt의 invariant는 바뀌지 않는다.
