...
- `BeginCleanupTransition`은 같은 transaction에서 `reserved`, `reservation_deadline <= now`, active lease 없음과 3-way linkage를 확인한다. 만료 forward lease에 recovery attempt count가 있으면 exact nested attempt를 검증해 `started`를 `failed/lease_expired`로 닫고, token을 정확히 1 증가시킨 뒤 lease를 제거해 `cleanup_pending`, `cleanup_mode=reservation_expiry`, `cleanup_origin_status=reserved`, quiet-period 기준을 함께 기록한다. Missing·malformed·completed prior attempt는 receipt transition도 0으로 거부한다. Deadline 전 recovery claim과 경쟁해도 둘 중 유효한 상태 전이 하나만 commit되며, deadline 뒤 recovery claim은 이 transaction보다 먼저 commit돼도 lease를 얻을 수 없다. 상세한 multi-clock·원장 원자성 결정은 [ADR-0022](./ADR-0022-atomic-cleanup-transition-attempt-closure.md)를 따른다.
