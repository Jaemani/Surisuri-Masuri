...
### 5. R8c execution result는 mutation 결과이지 receipt terminal 권한이 아니다

R8c component는 각 경로에 대해 bounded outcome을 반환한다.

- delete RPC outcome: `deleted_observed|not_found_observed|not_attempted|unknown`
- audit outcome: `confirmed_absent|present|soft_deleted|unavailable`
- 전체 disposition: `confirmed_empty|hold|retry`

Result에는 좌표, raw body, Firebase UID/App ID, credential과 provider 원문 오류를 넣지 않는다. Path와 digest는 server-only execution evidence에서만 다루고 사람 대상 리포트에는 원문을 복사하지 않는다.
