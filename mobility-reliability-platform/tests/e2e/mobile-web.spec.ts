...
test.describe('mobile web visual preview', () => {
  ...
  test('covers repair request, support, and repairer workspaces', async ({ page }) => {
    ...
    await page.getByRole('radio', { name: '작업 항목 1 브레이크' }).click();
    await page.getByRole('radio', { name: '작업 항목 1 교체' }).click();
    await page.getByLabel('작업 항목 1 금액').fill('85000');
    await page.getByRole('button', { name: '작업 항목 추가' }).click();
    await page.getByRole('radio', { name: '작업 항목 2 배터리' }).click();
    await page.getByLabel('작업 항목 2 수량').fill('2');
    await page.getByLabel('작업 항목 2 금액').fill('30000');
    await expect(page.getByText('115,000원')).toBeVisible();
    await page.getByRole('button', { name: '입력 내용 검토' }).click();
    await expect(page.getByRole('heading', { name: '제출 전 확인' })).toBeVisible();
    await expect(page.getByText('작업 항목 2건')).toHaveCount(0);
    await expect(page.getByText('2. 배터리 · 수리')).toBeVisible();
    await expect(page).toHaveScreenshot('mobile-repairer-submit-review.png', { animations: 'disabled', fullPage: true });
    await page.getByRole('button', { name: '수정하기' }).click();
    await expect(page.getByLabel('작업 항목 2 금액')).toHaveValue('30000');
    await page.getByRole('button', { name: '입력 내용 검토' }).click();
    await page.getByRole('button', { name: '이 내용으로 제출' }).click();
